...
Для Windows используйте специальный скрипт установки:

```bash
# Запустите в командной строке (cmd или PowerShell)
...
# Установите requirements вручную
pip install -r requirements-win.txt
```

**Устранение неполадок:**

- **Ошибка "faiss-cpu==1.7.4"**: Используйте `requirements-win.txt` - там указана совместимая версия `faiss-cpu==1.8.0`
- **Проблемы с dlib**: Скачайте wheel с https://pypi.org/project/dlib/#files или установите через conda
- **CUDA не найден**: Это нормально для CPU - приложение будет работать на процессоре
- **Много логов /api/image/preview**: Логи запросов превью изображений автоматически фильтруются для чистоты вывода
